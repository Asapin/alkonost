...
mod custom_deser_impls {
    ...
    impl<'de> Deserialize<'de> for VideoList {
        ...
        {
            impl Into<Option<String>> for GridShelfItem {
                fn into(self) -> Option<String> {
                    match self.grid_video_renderer.published_time_text {
                        Some(_) => None,
                        None => Some(self.grid_video_renderer.video_id),
                    }
                }
            }

            impl Into<Option<String>> for ShelfItem {
                fn into(self) -> Option<String> {
                    match self.video_renderer.published_time_text {
                        Some(_) => None,
                        None => Some(self.video_renderer.video_id),
                    }
                }
            }

            let outer = Outer::deserialize(deserializer)?;
            let streams = outer.contents
                .renderer
                .tabs
                .into_iter()
                .filter_map(|tab| {
                ...
                .map(|shelf_renderer: ShelfRenderer| {
                    let streams: HashSet<String> = match shelf_renderer.content {
                        ShelfContent::GridRenderer { 
                            items 
                        } | ShelfContent::HorizontalListRenderer { 
                            items 
                        } => {
                            items.into_iter()
                        ...
                        ShelfContent::VerticalListRenderer { 
                            items 
                        } => {
                            items.into_iter()
                                .filter_map(|item| item.into())
