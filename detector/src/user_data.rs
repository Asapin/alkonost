...
enum UserStatus {
    ...
    Blocked {                         // User was blocked by moderators or by the streamer
        history: Vec<(u64, String)>,  // But there's still a chance, that the ban was a mistake, and they can be unblocked
        delete_messages_count: usize,
    },
    Suspicious {                      // Collects data just as a regular user, but doesn't analyze it
        history: Vec<(u64, String)>,
        delete_messages_count: usize,
    },
    Regular {                         // Regular user. Collect and analyze their messages
        history: Vec<(u64, String)>,
        delete_messages_count: usize,
    },
}
    ...
    fn do_analysis(
        ...
        match status {
            ...
            UserStatus::Blocked { 
                ...
                let (new_status, decision) = UserData::do_analysis(temp_status, message, detector_params);
            ...
            UserStatus::Suspicious { 
                ...
                match message {
                    ...
                    UserMessage::Blocked => (UserStatus::Blocked { history, delete_messages_count }, Some(Decision::Blocked)),
                    UserMessage::Delete => {
                        delete_messages_count += 1;
                        (UserStatus::Suspicious { history, delete_messages_count }, None)
                    },
                    UserMessage::Regular { 
                        message, 
                        timestamp, 
                ...
                match message {
                    ...
                    UserMessage::Delete => {
                        ...
                        if detector_params.is_too_many_deleted_messages(delete_messages_count) {
                            return (UserStatus::Suspicious { history, delete_messages_count }, Some(Decision::TooManyDeleted));
                        }

                        (UserStatus::Regular { history, delete_messages_count }, None)
                    },
                    UserMessage::Regular { 
                        message, 
                        timestamp, 
                        ...
                        let result = history
                            .iter()
                            .fold(init, |mut acc, (timestamp, message)| {
                        ...
                        if detector_params.are_messages_too_long(current_avg_length, history.len()) {
                            let new_status = UserStatus::Suspicious { history, delete_messages_count };
