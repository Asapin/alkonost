...
impl UserData {
    ...
    pub fn new_message(&mut self, message: &str, timestamp: u64, detector_params: &DetectorParams) -> Option<SuspicionReason> {
        self.total_messages += 1;

        let message_length = message.chars().count();
        self.avg_message_length = (message_length as f32 + (self.total_messages - 1) as f32 * self.avg_message_length) / self.total_messages as f32;
        ...
        self.last_message_timestamp = timestamp;
        self.avg_delay = (time_diff + (self.total_messages - 1) * self.avg_delay as u64) as f32 / self.total_messages as f32;
